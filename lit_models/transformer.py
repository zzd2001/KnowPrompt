...
class BertLitModel(BaseLitModel):
    ...
    def training_step(self, batch, batch_idx):  # pylint: disable=unused-argument
        ...
        labels:（batchsize）
        so：subject和object开始和结束的token下标
        '''
        ...
        output_embedding = result.hidden_states[-1]  # 这是模型最后一层的隐藏状态，表示输入序列在经过所有层后得到的最终特征表示，result.hidden_states：这是一个包含每一层隐藏状态的列表或元组。每一层的隐藏状态通常是一个形状为 [batch_size, sequence_length, hidden_size] 的张量。
        logits = self.pvp(logits, input_ids)  # 模型的输出 logits 转换为特定标签的预测分数。具体来说，它将 [batch_size, seq_len, vocab_size] 形状的 logits 转换为 [batch_size, num_labels] 形状的输出
    ...
    def pvp(self, logits, input_ids):
        ...
        # self.word2label为对应标签词的词表索引，一共12个，自己加的，这样就可以去到预测这些标签词的概率值
        return final_output
    ...
    def ke_loss(self, logits, labels, so, input_ids):
        ...
        bsz = logits.shape[0]
        for i in range(bsz):
            subject_embedding.append(torch.mean(logits[i, so[i][0]:so[i][1]], dim=0))  # 以subject range的所有token的Embedding均值作为subject_embedding
